...
test('Adding and removing a release into a project in the database', async () => {
  ...
  await cw.projects.releases.add(project.id, release);

  let searchProject = await cw.projects.find({ id: project.id });
  expect(searchProject[0].releases[release.id].id).toEqual(release.id);

  await cw.projects.releases.delete(project.id, release.id);
...
test('Adding and removing a sprint into a project in the database', async () => {
  ...
  await cw.projects.sprints.add(project.id, sprint);

  let searchProject = await cw.projects.find({ id: project.id });
  expect(searchProject[0].sprints[sprint.id].id).toEqual(sprint.id);

  await cw.projects.sprints.delete(project.id, sprint.id);
...
test('Adding and removing a task into a project in the database', async () => {
  ...
  const task = {
    ...
    status: 'todo',
    name: 'Add Test Cases',
    description: 'Add more test cases using TDD principles',
    points: 10,
    takenBy: []
  };
  await cw.projects.tasks.add(project.id, task);

  let searchProject = await cw.projects.find({ id: project.id });
  expect(searchProject[0].tasks[task.id].id).toEqual(task.id);

  await cw.projects.tasks.delete(project.id, task.id);

  searchProject = await cw.projects.find({ id: project.id });
  expect(searchProject[0].tasks[task.id]).toBeUndefined();
});
